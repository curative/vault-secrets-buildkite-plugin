...
def generate_vault_request(awsIamServerId):
    ...
    request_dict = client._convert_to_request_dict({}, operation_model,endpoint_url="https://sts.amazonaws.com")
    ...
    return {
        ...
        'iam_request_body':        base64.b64encode(request.body.encode("utf-8")).decode("utf-8"),
        'iam_request_headers':     base64.b64encode(json.dumps(headers_to_go_style(dict(request.headers))).encode("utf-8")).decode("utf-8"), # It's a CaseInsensitiveDict, which is not JSON-serializable
    }
